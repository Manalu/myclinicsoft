<?php if(count($result) > 0) { ?>
	<table class="table">
		<thead>
			<tr>
				<th><p><?php echo ucfirst($type);?></p></th>
				<th>
		...
		<?php foreach($result as $row) { ?>
			<tr id="row-<?php echo $row['id'];?>">
				<td>
					<strong id="history-<?php echo $row['id'];?>"><?php echo $row['temperature'] ;?></strong></br>
					
					<span class="text-muted"><?php echo date($this->config->item('dateformat'), strtotime($row['date']));?></span>
					
				</td>
				<td>
					<a href="<?php echo site_url('records/delete/'.$row['id'].'/'.$type);?>" id="<?php echo $row['id'];?>" class="pull-right delete btn btn-danger btn-xs"><i class="fa fa-trash-o"></i></a>
				</td>
			</tr>
		<?php } ?>
		</tbody>
