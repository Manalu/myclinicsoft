...
class Tank_auth
{
	...
	function set_user_data($user, $login, $remember){
	...
	{
		return $this->ci->session->userdata('license_key');
	}
	/**
	...
	function create_user($username, $email, $password, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			...
			$data = array(
				...
				'email'		=> $email,
				'role_id'	=> 2,//$this->ci->users->get_default_role()->id,
				'last_ip'	=> $this->ci->input->ip_address(),
				'license_key'   => md5(random_string('alnum',10)),
			);
